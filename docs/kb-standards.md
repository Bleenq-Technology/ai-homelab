...
> **Interim bridge (live 2026-06-18, before the Qdrant MCP):** OpenWebUI reaches the **whole KB
> library** via an **OpenWebUI Pipe function** ("KB: Bleenq Knowledge") that calls the n8n `kb-chat`
> agent workflow — see `docker/ai/openwebui/functions/`. That agent holds **one retrieval tool per
> KB** (descriptions from the manifest, §6), so it enumerates the KBs and routes each question to the
> right collection(s), doing the `bge-m3`+Qdrant retrieval itself. OpenWebUI thus gets shared-KB
> answers without its Knowledge UI. The Qdrant MCP (§5/todos #4) generalises this to all agents.
...
> **Implemented (2026-06-18):** the registry is **`docs/kb-manifest.json`** — the version-controlled
> source of truth (name, description, collection, source repo, include filter, `enabled`). It drives
> **both** the n8n ingest workflow and the chat agent (which bakes each KB's description into its
> system prompt for routing/enumeration) — generated by `docker/ai/n8n/build_kb_workflows.py`. When the
> Qdrant MCP (§5) lands, its `list_kbs()` reads this same file. Migrate to Baserow only if non-devs
> need to edit it.
...
- **Git-native** (cleanest): for KBs whose source already lives in a repo (`kb_homelab_docs`,
  ...
  **Implemented (2026-06-18) — manifest-driven, multi-repo** (native n8n nodes, not a Code node): one
  `kb-ingest` workflow generates a *clear → GitHub Document Loader (markdown-only via `ignorePaths`) →
  Recursive Char Text Splitter → `bge-m3` Embeddings → Qdrant insert* chain **per enabled KB** in
  `docs/kb-manifest.json` (currently `kb_homelab_docs`, `kb_trading_docs`, `kb_design_docs`), on a
  daily 04:00 + manual schedule. See `docker/ai/n8n/workflows/`. **Caveat:** the native vector-store
  insert appends with random point ids (no §4 hash dedup yet), so the flow **clears + rebuilds** each
  collection every run — fine for these small markdown repos; revisit if a source grows.
