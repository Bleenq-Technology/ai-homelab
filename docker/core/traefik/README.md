...
**Auth:** basic-auth (dashboard only) — deliberate break-glass, not Keycloak
...
- Dashboard router (`dashboard`): `Host(traefik.${DOMAIN})`, `websecure`, `service=api@internal`, middlewares `dashboard-auth@docker,secure-chain@file`. `dashboard-auth` is a basicauth middleware whose users come from `TRAEFIK_DASHBOARD_AUTH` (apr1 hash).
...
## Secrets
- `.env` keys: `TRAEFIK_DASHBOARD_AUTH` (apr1 hash), `DOMAIN`.
